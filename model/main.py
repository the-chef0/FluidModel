...
def main(args: argparse.Namespace, activeloop: bool = True) -> None:
    ...
    resnet_model = torch.load(r'transfer_models\resnet_model.pth')

    # Load the EffientNetV2 model
    eff_model = torch.load(r"transfer_models\efficientnet_model.pth")

    # Load the Neural Net
    custom_model = Net(n_classes=5)

    # Load the train and test data set (change path to whatever data is stored in)
    train_dataset = ImageDataset(Path("data/X_train.npy"), Path("data/Y_train.npy"))
    test_dataset = ImageDataset(Path("data/X_test.npy"), Path("data/Y_test.npy"))

    # Create dictionary of models, so it can be selected by the user with the --model argument
    models = {"resnet": resnet_model, "efficientnet": eff_model, "custom": custom_model}

    # Load the Neural Net
    model = models[args.model]
    image_shape = (1, 710, 690)
    ...
    # Moving our model to the right device (GPU/CPU):
    if torch.cuda.is_available():
        print("@@@ CUDA device found, enabling CUDA training...")
        device = "cuda"
        model.to(device)
        # Creating a summary of our model and its layers:
        summary(model, image_shape, device=device)
    elif (
        torch.backends.mps.is_available()):  # PyTorch supports Apple Silicon GPU's from version 1.12
    ...
    # Train and test our model for multiple epochs:
    train_sampler = BatchSampler(
        batch_size=batch_size, dataset=train_dataset, balanced=args.balanced_batches
    )
    test_sampler = BatchSampler(
        batch_size=32, dataset=test_dataset, balanced=args.balanced_batches
    )
